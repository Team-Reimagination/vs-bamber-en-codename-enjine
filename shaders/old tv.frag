...
float vignette(vec2 uv, float size, float smoothness, float edgeRounding)
{
    ...
    amount = 0.9 - amount; //black border counting down is stronger
...
void mainImage()
{
    ...
    vec4 col;
    col.r = texture(iChannel0, crt_uv).r;
    col.g = texture(iChannel0, crt_uv).g;
    col.b = texture(iChannel0, crt_uv ).b;
    col.a = texture(iChannel0, crt_uv).a;
    
    col = mix(col, vec4(s1 + s2), 0.05);
//final value here is 0.05 originally for bar thickness
//crt_uv + vec2(0., 0.01) for rgb sineoutline effect
